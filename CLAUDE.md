...
- `QueryLimitsTest` (`@TestProfile` tightening `jdbc.query.max-rows` and `jdbc.query.timeout`) — the truncation envelope, and that an aborted query leaves the session usable. `SessionContentionTest` (`@TestProfile` disabling `jdbc.query.timeout`) — a second call must be able to take over a session held by a stuck query, driven from a background thread so both calls share one MCP session id.
...
- **The lock wait is bounded, and contention cancels the holder.** `acquire()` uses `tryLock(jdbc.session.lock-timeout)`, not `lock()`. Past the timeout it calls `Statement.cancel()` on the statement the holder published through `Lease.track(...)` and waits once more. This is deliberate: the lock is only ever held by a running statement, so a holder that outlasts the wait is either an abandoned call whose MCP client already timed out, or a query too slow to be useful. Blocking on it made the whole server appear dead — every later call, `SELECT 1` included, queued behind it until the query finished or the process was restarted, because the idle sweeper's `tryLock` skips locked sessions and `lastUsedNanos` is only refreshed on release. `SessionContentionTest` covers the recovery; do not turn the wait back into an unbounded `lock()`.
- **`@PreDestroy` must never block on a running call** either: it cancels the active statement, waits a few seconds for the lock, then closes the connection whether it got the lock or not.
...
| `jdbc.session.lock-timeout` | How long a tool call waits for a busy session before cancelling the statement holding it (ISO-8601 duration) | `PT30S` |
| `jdbc.session.cancel-on-contention` | Whether a waiting call may cancel the statement blocking its session; `false` fails the waiter instead | `true` |
| `jdbc.query.timeout` | `Statement.setQueryTimeout` applied to every statement, in seconds; `0` disables | `120` |
| `jdbc.query.max-rows` | Row cap for `read_query`; an over-limit result comes back as `{rows_truncated, row_limit, rows}` instead of a bare array. `0` disables | `1000` |
